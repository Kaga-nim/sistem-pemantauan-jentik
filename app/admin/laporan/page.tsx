...
export default function LaporanPage() {
  ...
  const load = () => {
    setLoading(true);
    let url = `/api/laporan?minggu=${filterMinggu}&tahun=${filterTahun}`;
  ...
  useEffect(() => { load(); }, [filterMinggu, filterTahun, filterWilayah]);

  const exportExcel = async () => {
    const { utils, writeFile } = await import("xlsx");

    const rows: (string | number | boolean)[][] = [
      ["No", "Nama Warga", "Wilayah", "Titik Pemeriksaan", "Ada Jentik", "Catatan Titik", "Catatan Umum", "Tanggal Konfirmasi"],
    ];

    let no = 1;
    for (const k of data) {
      if (k.detail_konfirmasi.length === 0) {
        rows.push([no++, k.nama_warga, k.wilayah?.nama || "", "", "", "", k.catatan || "", new Date(k.tanggal_konfirmasi).toLocaleString("id-ID")]);
      } else {
    ...
    ws["!cols"] = [{ wch: 5 }, { wch: 20 }, { wch: 20 }, { wch: 25 }, { wch: 12 }, { wch: 25 }, { wch: 25 }, { wch: 20 }];
    const wb = utils.book_new();
    utils.book_append_sheet(wb, ws, "Laporan");
    writeFile(wb, `laporan-jentik-minggu${filterMinggu}-${filterTahun}.xlsx`);
  };

  // Hitung pilihan minggu (52 minggu ke belakang)
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            {loading ? "Memuat..." : `${data.length} konfirmasi ditemukan`}
          </p>
        </div>
        <button
          onClick={exportExcel}
                      ...
                      <tr key={`${k.id}-detail`}>
                        <td colSpan={5} className="px-5 py-3 bg-gray-50">
                          <div className="text-xs text-gray-600 space-y-1">
                            ...
                            {k.catatan && (
                              <p className="text-gray-400 mt-1 italic">Catatan: {k.catatan}</p>
                            )}
                          </div>
                        </td>
                      </tr>
